                ...
                <div className="relative">
                    ...
                    <div className="absolute bottom-3 right-3 flex items-center gap-1">
                        <input
                            ...
                            accept="image/*"
                        />
                        <Button size="icon" onClick={() => onGenerate(prompt, 'tailwindcss', imageUrl || undefined)} disabled={isLoading || isEnhancing} className="rounded-lg">
                            <ArrowUp />
                        </Button>
                    </div>
                </div>
...
// THIS COMPONENT IS NO LONGER USED. IT HAS BEEN REPLACED BY `PageContent`
export function MainLayout() {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [framework, setFramework] = React.useState<Framework>('tailwindcss');
  ...
  React.useEffect(() => {
    ...
    if (promptFromUrl) {
      ...
      onGenerate(decodedPrompt, 'tailwindcss');
    }
